...
structure SolverState where
  ...
  args : Std.HashMap Kind (Array String)
...
def create (timeoutSecs : Nat) (solvers : List Kind): IO SolverState := do
  let allArgs : Std.HashMap Kind (Array String) := Std.HashMap.ofList [
    ...
    (.mmard,     #["--timeout", toString timeoutSecs]),
    (.mmafi,     #["--timeout", toString timeoutSecs]),
  ]
